...
SIM_DAYS = 360               # 取引シミュレーションを実行する日数
HOLD_PERIOD = 12            # 仮想取引でポジションを保持する時間（時間）
FEE_RATE = 0.0015           # 取引手数料の割合
CONFIDENCE_THRESHOLD = 0.60 # 「買い」判断を行うための予測信頼度の閾値 (60%)
SIM_OFFSET_DAYS = 60        # シミュレーション開始日を過去にずらす日数
STOP_LOSS_THRESHOLD = 0.03  # 損切りを行う価格下落率の閾値 (5%)
